...
Use \Gate\Package\User\Helper\RedisUserConnectHelper;
Use \Gate\Package\User\Helper\RedisUserStatisticHelper;
Use \Gate\Package\User\Helper\CacheUserHelper;
Use \Gate\Package\User\Helper\DBUserHelper;
Use \Gate\Libs\Cache\Memcache;
Use \Gate\Libs\Base\ZooClient;
Use \Gate\Libs\Base\GateLog;
Use \Gate\Libs\Base\HttpRequest;
Use \Gate\Libs\Base\Utilities;
Use \Gate\Package\Session\UserSession;
...
Use \Gate\Package\Oauth\RenrenOauth;
Use \Gate\Package\Oauth\RenrenClient;
Use \Gate\Package\Oauth\WYOAuth;
Use \Gate\Package\Oauth\WYOAuth2;
Use \Gate\Package\Oauth\WYClient;
Use \Gate\Package\Oauth\TX_OAuth2;
Use \Gate\Package\Oauth\TX_WeiboClient;
Use \Gate\Package\Oauth\Taobao_sdk\TopSdk;
Use \Gate\Package\Oauth\Taobao_sdk\Top\TopClient;
Use \Gate\Package\Oauth\Baidu;
Use \Gate\Package\Oauth\BaiduSessionStore;
Use \Gate\Package\Oauth\Baidu_pcs\BaiduPCS;
Use \Gate\Package\Oauth\DouBanPublic;
Use \Gate\Package\Oauth\DouBanClient2;
Use \Gate\Package\Oauth\DouBanOAuth2;
...
class UserConnect {
	...
	public function getUserConnectFromDB($param = array(), $selectColumn = "*" , $master = FALSE, $indexKey = '') {
        $sqlData = array();
        $sqlComm = "SELECT $selectColumn FROM dolphin_user_profile_connect WHERE 1=1 ";
    ...
    public function updateUserConnectAuth($userId, $type, $auth, $status, $access) {
        $sqlComm = "update dolphin_user_profile_connect set auth=:_auth, status=:_status, access=:_access where user_id=:_user_id and user_type=:_user_type";
    ...
    public function updateUserConnectSettings($userId, $type, $settings) {
        ...
        $sqlComm = "update dolphin_user_profile_connect set
	...
	/**
	 * 插入用户互联表信息dolphin_user_profile_connect
	 * @author Chen Hailong
	 * @param int $userId
	 * @param int $userType
	 *		1: renren, 3: weibo, 4: qzone, 5: baidu, 6: taobao, 7: wangyi,
    ...
    function insertUserConnectInfo($userId, $userType, $auth, $access = '') {
        ...
        $sqlComm = "INSERT INTO dolphin_user_profile_connect" .
	...
	public function checkQzoneIsFans($userId) {
		...
		//$request = new \Gate\Libs\Base\HttpRequest();
    ...
    public static function getWeiboInfo($keyInfo = array(WB_AKEY, WB_SKEY), $weibo_access_keys = NULL) {
        ...
        if (isset($ret['error'])) {
			...
			if (isset($userInfo['error'])) {
				$logHandle = new \Gate\Libs\Base\GateLog('get_weibo_info_api', 'normal');
    ...
    public static function weiboAuth($callbackUrl, $keyInfo = array(WB_AKEY, WB_SKEY), $fromMobile = FALSE, $requestCode) {
        ...
        if (!isset($requestCode)) { //$_REQUEST['code'])) {
            ...
            if (is_array($last_key)) {
                ...
                $logHandle = new GateLog( "sinaWeiboOAuth2", "normal" );
    ...
    public static function wangyiAuth2($callbackUrl, $keyInfo = array(WY_AKEY_NEW, WY_SKEY_NEW), $requestCode, $state) {
        ...
        if (!isset($requestCode)) { 
            ...
            if (is_array($last_key)) {
                ...
                $logHandle = new GateLog( "wangyiOAuth2", "normal" );
                $logHandle->w_log(print_r($last_key, true));
                return FALSE;
            }
			$result = array();
	...
	public static function getQzoneInfo($access_token = NULL, $openId = NULL, $keyInfo = array(QZONE_ID, QZONE_KEY)) {
        ...
        if (!empty($qc)) {
			...
			if ($userInfo['ret'] !== 0) {
				//$logHandle = new \Gate\Libs\Base\GateLog('qzone_info_api', 'normal');
				//$logHandle->w_log(print_r($userInfo, TRUE));
				return FALSE;
			}
        }    
	...
	public static function taobaoAuth($callback, $keyInfo = array(TAOBAO_APPKEY_AUTH, TAOBAO_APPSECRET_AUTH), $params = array()) {
        ...
        if (!empty($params['code']) && !empty($params['state']) && $params['state'] == 1) { 
            ...
            $logHandle = new \Gate\Libs\Base\GateLog("taobaoError", "normal");
    ...
    public static function doubanAuth2($callbackUrl, $keyInfo = array(DOUBAN_AKEY_NEW, DOUBAN_SKEY_NEW), $requestCode, $state) {
        ...
        if (!isset($requestCode)) { 
            ...
            if (is_array($last_key)) {
                ...
                $logHandle = new GateLog( "doubanOAuth2", "normal" );
                $logHandle->w_log(print_r($last_key, true));
                return FALSE;
            }
			$result = array();
