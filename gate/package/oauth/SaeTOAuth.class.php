...
class SaeTOAuth {
	...
	function getAuthorizeURL( $url, $response_type = 'code', $settings = '') {
		$params = array();
		...
		if (!empty($settings) && ($settings === 'settings' || $settings === 'bind')) {
			$params['state'] = $settings;
		}
		elseif (!empty($settings) && $settings === TRUE) {
			$params['display'] = 'mobile';
		}
		if (!empty($settings) && is_array($settings)) {
			$settings = implode('__', $settings);
			$params['state'] = $settings;
		}
		else {
			$params['state'] = $settings;
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
            ...
            $logHandle = new GateLog('error_weibo', 'normal');
