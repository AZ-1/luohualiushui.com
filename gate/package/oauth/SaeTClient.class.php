<?php
namespace Gate\Package\Oauth;

Use \Gate\Package\Oauth\SaeTOAuth AS SaeTOAuth;
Use \Gate\libs\Cache\Memcache;
/**
 * 新浪微博操作类
 *
...
class SaeTClient
{
	...
	function public_timeline( $count = 20, $base_app = 0 )
	{
		...
		return $this->oauth->get('https://api.weibo.com/2/statuses/public_timeline.json', $params);
	}
	...
	function home_timeline( $page = 1, $count = 20, $since_id = NULL, $max_id = NULL, $base_app = 0, $feature = 0 )
	{
		...
		return $this->request_with_pager('https://api.weibo.com/2/statuses/home_timeline.json', $page, $count, $params );
	}
	...
	function mentions( $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/statuses/mentions.json' , $page , $count, $params );
	}
	...
	function update( $status, $reply_id = NULL, $lat = NULL, $long = NULL, $annotations = NULL )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/statuses/update.json' , $params );
	}
	...
	function upload( $status , $pic_path, $lat = NULL, $long = NULL )
	{
		...
		$log = new zx_log('uploadlog', 'normal');
		//stream_get_contents效率要高于file_get_contents
        if ($stream = fopen($pic_path, 'rb')) {
           $content = stream_get_contents($stream);
           //$log->w_log('content: ' . $content . "\t");
           fclose($stream);
        }
		if (empty($content)) {
           $log->w_log('empty content ' . "\t");
			return FALSE;
		}
		...
		return $this->oauth->post( 'https://upload.api.weibo.com/2/statuses/upload.json' , $params , true );
	}
	...
	function show_status( $sid )
	{
		$this->id_format($sid);
		$params['id'] = $sid;
		return $this->oauth->get( 'https://api.weibo.com/2/statuses/show.json', $params );
	}
	...
	function destroy( $sid )
	{
		$this->id_format($sid);
		$param['id'] = $sid;
		return $this->oauth->post( 'https://api.weibo.com/2/statuses/destroy.json', $param );
	}
	...
	function show_user( $uid_or_name )
	{
		...
		$param['uid'] = $uid_or_name;
		return $this->oauth->get( 'https://api.weibo.com/2/users/show.json' ,  $param);
	}
	...
	{
		return $this->request_with_uid( 'https://api.weibo.com/2/friendships/friends.json' ,  $uid_or_name , NULL , $count , $cursor );
	}
	 ...
	 * @param int $count 每次返回的最大记录数（即页面大小），不大于200,默认返回20。可选。
	 * @param mixed $uid_or_name 要获取粉丝的 UID或微博昵称。
	 * @return array
	 */
	function followers($uid = NULL, $cursor = NULL , $count = NULL, $access_token = NULL)
	{
		$params = array();
		$params['uid'] = intval($uid);
		//$params['access_token'] = $access_token;
		$params['cursor'] = isset($cursor) ? $cursor : 0;
		$params['count'] = isset($count) ? $count : 50;

		return $this->oauth->get( 'https://api.weibo.com/2/friendships/followers.json' , $params );
		//return $this->request_with_uid( 'https://api.weibo.com/2/friendships/followers.json' ,  $uid_or_name , NULL , $count , $cursor );
	}
	...
	{
		return $this->request_with_uid( 'https://api.weibo.com/2/friendships/create.json' ,  $uid_or_name ,  NULL , NULL , NULL , true  );
	}
	...
	{
		return $this->request_with_uid( 'https://api.weibo.com/2/friendships/destroy.json' ,  $uid_or_name ,  NULL , NULL , NULL , true);
	}
	...
	function update_remark( $uid, $remark )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/friendships/remark/update.json' , $params );
	}
	...
	function hot_users( $category = "default" )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/suggestions/users/hot.json' , $params );
	}
	...
	function emotions( $type = "face", $language = "cnname" )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/emotions.json' , $params );
	}
	...
	function reset_count( $type )
	{
		...
		return $this->oauth->post( 'https://rm.api.weibo.com/2/remind/set_count.json' , $params );
	}
	...
	function is_followed( $target, $source = NULL )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/friendships/show.json' , $params );
	}
	...
	function list_dm( $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/direct_messages.json' , $page , $count, $params );
	}
	...
	function list_dm_sent( $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/direct_messages/sent.json' , $page , $count, $params );
	}
	...
	function send_dm( $uid_or_name , $text )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/direct_messages/new.json' , $params  );
	}
	...
	{
		$this->id_format($did);
		return $this->oauth->post( 'https://api.weibo.com/2/direct_messages/destroy/' . $did . '.json' );
	}
	...
	function delete_dms( $dids )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/direct_messages/destroy_batch.json' , $params );
	}
	...
	function repost_by_me( $uid, $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/statuses/repost_by_me.json' , $page , $count , $params );
	}
	/** 
     * 获取与自己互相关注的成员列表
     * 
     * @param integer $uid 用户的微博id 
     */
    function bilateral( $uid, $count = 200, $page = 1 )
    {   
        $param = array();
        $param['uid'] = $uid;
        $param['count'] = $count;
...
//		return $this->oauth->get('statuses/bilateral_timeline' , $param );
        return $this->oauth->get('friendships/friends/bilateral', $param);
    }   	
	/**
	 * 返回一条原创微博的最新n条转发微博信息
	 *
	...
	function repost( $sid , $text = NULL, $is_comment = 0 )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/statuses/repost.json' , $params  );
	}
	...
	function send_comment( $sid , $text , $cid = NULL )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/comments/create.json' , $params  );
	...
	function comment_destroy_batch( $cids )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/comments/destroy_batch.json' , $params );
	}
	...
	function unread( $with_new_status = 0, $since_id = NULL )
	{
		...
		return $this->oauth->get( 'https://rm.api.weibo.com/2/remind/unread_count.json' , $params );
	}
	...
	function reply( $sid , $text , $cid )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/comments/reply.json' , $params);
	...
	function get_favorites( $page = NULL )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/favorites.json', $params );
	}
	...
	function comment_destroy( $cid )
	{
		...
		$param = array();
		$param['cid'] = $cid;
		return $this->oauth->post( 'https://api.weibo.com/2/comments/destroy.json', $param );
	}
	...
	function comments_to_me( $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/comments/to_me.json' , $page , $count , $params );
	}
	...
	function comments_by_me( $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/comments/by_me.json' , $page , $count , $params );
	}
	...
	function comments_timeline( $page = 1 , $count = 20, $since_id = NULL, $max_id = NULL )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/comments/timeline.json' , $page , $count , $params );
	}
	...
	function get_comments_by_sid( $sid , $page = 1 , $count = 20 )
	{
		...
		return $this->oauth->get('https://api.weibo.com/2/comments/show.json' , $params );
	...
	function get_count_info_by_ids( $sids )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/comments/show_batch.json' , $params );
	}
	...
	function add_to_favorites( $sid )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/favorites/create.json' , $params );
	}
	...
	function remove_from_favorites_batch( $fids )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/favorites/destroy_batch.json' , $params );
	}
	...
	function remove_from_favorites( $id )
	{
		$this->id_format($id);
		$param['id'] = $id;
		return $this->oauth->post( 'https://api.weibo.com/2/favorites/destroy.json', $param );
	}
	...
	{
		return $this->oauth->get('https://api.weibo.com/2/account/get_uid.json');
	}
	...
	{
		return $this->oauth->get('https://api.weibo.com/2/account/rate_limit_status.json');
	}
	...
	{
		return $this->oauth->post('https://api.weibo.com/2/account/end_session.json');
	}
	 ...
	 *  - comment: 谁可以评论此账号的微博。 0：所有人 1：我关注的人 默认为0
	...
	{
		return $this->oauth->post('https://api.weibo.com/2/account/update_privacy.json', $privacy_settings);
	}
	...
	{
		return $this->oauth->post('https://api.weibo.com/2/account/get_privacy.json');
	}
	...
	/*function update_profile_image($image_path)
	{
		...
		return $this->oauth->post('https://api.weibo.com/2/account/update_profile_image.json', $params, true);
	}*/
	...
	{
		return $this->oauth->post('https://api.weibo.com/2/account/update_profile.json', $profile);
	}
	...
	function provinces()
	{
		return $this->oauth->get('https://api.weibo.com/2/provinces.json');
	}
	...
	{
		return $this->request_with_uid( 'https://api.weibo.com/2/friendships/friends/ids.json' ,  $uid_or_name , false , $count , $cursor );
	}
	...
	{
		return $this->request_with_uid( 'https://api.weibo.com/2/friendships/followers/ids.json' ,  $uid_or_name , false , $count , $cursor );
	}
	...
	function add_to_blocks( $user_id = NULL, $screen_name = NULL )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/blocks/create.json' , $params );
	}
	...
	function remove_from_blocks( $user_id = NULL, $screen_name = NULL )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/blocks/destroy.json' , $params );
	}
	...
	function in_blocks( $user_id = NULL, $screen_name = NULL )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/blocks/exists.json' , $params );
	}
	...
	{
		return $this->request_with_pager( 'https://api.weibo.com/2/blocks/blocking.json' , $page , $count );
	}
	...
	{
		return $this->request_with_pager( 'https://api.weibo.com/2/blocks/blocking/ids.json' , $page , $count );
	}
	...
	function get_tags( $user_id = NULL, $page = 1, $count = 20 )
	{
		...
		return $this->request_with_pager( 'https://api.weibo.com/2/tags.json' , $page , $count , $params );
	}
	...
	function add_tags( $tags )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/tags/create.json' , $params );
	}
	...
	{
		return $this->request_with_pager( 'https://api.weibo.com/2/tags/suggestions.json' , $page , $count );
	}
	...
	function delete_tag( $tag_id )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/tags/destroy.json' , $params );
	}
	...
	function delete_tags( $ids )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/tags/destroy_batch.json' , $params );
	}
	...
	function get_trends( $user_id = NULL, $page = 1, $count = 20 )
	{
		...
		$this->id_format($params['user_id']);
		return $this->request_with_pager( 'https://api.weibo.com/2/trends.json' , $page , $count , $params );
	}
	...
	function follow_trends( $trend_name )
	{
		...
		return $this->oauth->post( 'https://api.weibo.com/2/trends/follow.json' , $params );
	}
	...
	function unfollow_trends( $tid )
	{
		...
		return $this->oauth->delete( 'https://api.weibo.com/2/trends/destroy.json' , $params );
	}
	...
	function hourly_trends( $base_app = 1 )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/trends/hourly.json' , $params );
	}
	...
	function daily_trends( $base_app = 1 )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/trends/daily.json' , $params );
	}
	...
	function weekly_trends( $base_app = 1 )
	{
		...
		return $this->oauth->get( 'https://api.weibo.com/2/trends/weekly.json' , $params );
	}
	 ...
	 */
	function send_notice($params) {
		return $this->oauth->post('https://api.weibo.com/2/notification/send.json', $params);
	}
	 ...
	 *
	 */
	function get_short_url($url_long, $app_akey) {
		$params = array('url_long' => $url_long);
		$params['source'] = $app_akey;
		$cache = Memcache::instance();
		$short_url = $cache->get('short_url:' . md5($url_long));
		if (!empty($short_url)) {
			return $short_url;
		}
		$return = $this->oauth->get('https://api.weibo.com/2/short_url/shorten.json' , $params);
		if (!empty($return[0]['url_short'])) {
			$cache->set('short_url:' . md5($url_long), $return[0]['url_short'], 684000);	
	...
	//批量获取用户的粉丝数、关注数、微博数
	function count_user_statistic($uids = array()) {
		$params = array();
	 ...
	 * 通过微博（评论、私信）ID获取其MID
	 */
	function query_mid($id, $type = 1, $isbatch = 0) {
		$params = array();
		$params['id'] = $id;
		$params['type'] = $type;
		$params['is_batch'] = $isbatch;
		return $this->oauth->get('https://api.weibo.com/2/statuses/querymid.json', $params);
	}

	/**
	 * 创建一条新的行为动态（第三方写动态接口）
	 * $params $object_id 行为操作的实体唯一标识ID，格式为p:sid
	 * $params $object 行为操作的实体扩展元数据，JSON字符串，不超过768个字节
	 * $params $target_id 操作实体的id，格式参照object_id
	 * $params $target 操作实体的目标扩展元数据，JSON字符串，不超过768个字节
	 * $params $content 行为动态的文本描述，不超过140个汉字
	 */
	function activities_apps_update($tpl_id, $object_id, $object, $target_id = '', $target = '', $content) {
		$params = array();
		...
		return $this->oauth->post('https://api.weibo.com/2/activities/apps/update.json', $params);
	}
}
