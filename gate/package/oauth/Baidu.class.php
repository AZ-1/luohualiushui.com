...
class Baidu
{
    ...
    public function __construct($apiKey, $apiSecret, $store = null, $santor, $callback)
    {
    	$this->apiKey = $apiKey;
		...
		if (!empty($santor)) {
			...
			$cacheKey = 'baidu_code' . ':' . $this->santor;
			$this->keys = $cacheObj->get($cacheKey);
			//if (!empty($this->keys['code'])) {
				//var_dump($this->keys['code'], $this->keys['state']);
			//}
		}
    }
     ...
     * @return array
     */
	public function getSession($flag = FALSE)
	{
		//print_r($this->keys);
		//print_r($this->session);echo "\n";
		if ($this->session === null || $flag) {
			//print_r($this->keys);
	...
	public function getCurrentUrl()
	{
		...
		//$currentUrl = $protocol . $host . '/connect/auth/baidu';
		$currentUrl = $this->callback_url;
	...
	{
		//print_r($this->keys);echo "$$\n";
		if (isset($this->keys['code'])) {
			if ($this->state !== null &&
				isset($this->keys['state']) &&
				$this->state === $this->keys['state']) {
