<?php
namespace Gate\Package\Oauth\Baidu_pcs;

Use \Gate\Package\Oauth\Baidu_pcs\RequestCore;
Use \Gate\Package\Oauth\Baidu_pcs\ResponseCore;
/***************************************************************************
 * 
...
class BaiduPCS {
    ...
    public function __construct($auth) {
        ...
        if ((!isset($auth['access_token']) || empty($auth['access_token'])) &&
			...
			$logHandle = new \Gate\Libs\Base\GateLog('baidu_pcs', 'normal');
			$logHandle->w_log(print_r('miss access token or cookie in ', true));
        ...
        if (isset($auth['access_token'])) {
			...
			$logHandle = new \Gate\Libs\Base\GateLog('baidu_pcs', 'normal');
			$logHandle->w_log(print_r('miss access token or cookie', true));
    ...
    private function authenticate($requesttype, $method, $opt = NULL) {
        ...
        if (is_array($arr_body)) {
        	if (isset($arr_body['error_code'])) {
				$logHandle = new \Gate\Libs\Base\GateLog('baidu_pcs', 'normal');
				$logHandle->w_log(print_r($arr_body, TRUE));
    ...
    private function format_url($opt) {
        if (!isset($opt['request_type'])) {
			$logHandle = new \Gate\Libs\Base\GateLog('baidu_pcs', 'normal');
			$logHandle->w_log(print_r('miss pcs request type', true));
        ...
        if ((self::account_type != $opt['request_type']) && 
			...
			$logHandle = new \Gate\Libs\Base\GateLog('baidu_pcs', 'normal');
			$logHandle->w_log(print_r('invalid pcs request type', true));
    ...
    private function set_request($request, $opt) {
        ...
        if (!isset($opt['method'])) {
			$logHandle = new \Gate\Libs\Base\GateLog('baidu_pcs', 'normal');
			$logHandle->w_log(print_r('miss request method', true));
