...
class RequestCore {
	...
	public function prep_request() {
		...
		$bolRes = $this->isBaeEnv ();
		$curl_handle = NULL;
		if ($bolRes === true) {
			if (isset ( $_SERVER ['DOCUMENT_ROOT'] )) {
				$root = rtrim ( $_SERVER ['DOCUMENT_ROOT'], '/' );
				$filePath = $root . '/app_config.php';
				if (file_exists ( $filePath )) {
					require_once ($filePath);
					$curl_handle = bae_fetch_url_init ( BD_APP_FETCH_URl_PWD, BD_APP_FETCH_URl_USER );
				} else {
					trigger_error ( $filePath . 'does not exist' );
					exit ( 1 );
				}
			} else {
				trigger_error ( '$_SERVER["DOCUMENT_ROOT"] is not setted in bae environment,' . ' please contact the administrator' );
				exit ( 1 );
			}
		} else {
	...
	public function process_response($curl_handle = null, $response = null) {
		...
		if (is_resource ( $this->curl_handle )) {
			...
			if ($curl_handle && $response) {
				$result = new ResponseCore( $this->response_headers, $this->response_body, $this->response_code, $this->curl_handle );
				return $result;
			}
		}
		// Return false
