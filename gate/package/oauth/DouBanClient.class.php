...
class DouBanClient
{
	...
	public function fetchToken($oauthRequest = NULL) {
		$url = $oauthRequest->http_url();
		$headers = $oauthRequest->to_header();
		$a_header = explode(',', $headers['Authorization']);
		
		foreach($a_header as $value) {
			$s = str_replace('"', '', $value);
			$s = str_replace('=', ':', $s);
			$header[] = $s;
		}
...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod {/*{{{*/
  ...
  public function build_signature($request, $consumer, $token) {/*{{{*/
	...
	//print_r($key_parts);die('@@');
    $key_parts = array_map('\Gate\Package\Oauth\OAuthUtil::urlencodeRFC3986', $key_parts);
...
class OAuthRequest {/*{{{*/
  ...
  public function get_signable_parameters() {/*{{{*/
    ...
    $keys = array_map(array('\Gate\Package\Oauth\OAuthUtil', 'urlencodeRFC3986'), array_keys($params));
    $values = array_map(array('\Gate\Package\Oauth\OAuthUtil', 'urlencodeRFC3986'), array_values($params));
  ...
  public function get_signature_base_string() {/*{{{*/
	...
	//print_r($parts);
    $parts = array_map(array('\Gate\Package\Oauth\OAuthUtil','urlencodeRFC3986'), $parts);
	//print_r($parts);die('AA');
