...
class OAuthRequest {
    ...
    public function to_header() {
        ...
        foreach ($this->parameters as $k => $v) {
            ...
            if (is_array($v)) {
                throw new wqqOAuthException('Arrays not supported in headers');
            }
            $out .= ',' .
...
class OAuthServer {
    ...
    private function get_version(&$request) {
        ...
        if ($version && $version != $this->version) {
            throw new wqqOAuthException("OAuth version '$version' not supported");
        }
        return $version;
    }
    ...
    private function get_consumer(&$request) {
        ...
        if (!$consumer_key) {
            throw new wqqOAuthException("Invalid consumer key");
        }

        $consumer = $this->data_store->lookup_consumer($consumer_key);
        if (!$consumer) {
            throw new wqqOAuthException("Invalid consumer");
        }
    ...
    private function get_token(&$request, $consumer, $token_type="access") {
        ...
        if (!$token) {
            throw new wqqOAuthException("Invalid $token_type token: $token_field");
        }
        return $token;
    }
    ...
    private function check_signature(&$request, $consumer, $token) {
        ...
        if (!$valid_sig) {
            throw new wqqOAuthException("Invalid signature");
        }
    }
    ...
    private function check_nonce($consumer, $token, $nonce, $timestamp) {
        ...
        if ($found) {
            throw new wqqOAuthException("Nonce already used: $nonce");
        }
    }
...
class TX_WeiboOAuth {
   ...
   function requestTokenURL() { return 'https://open.t.qq.com/cgi-bin/request_token'; }
    ...
    function getRequestToken($oauth_callback = NULL) {
		...
		//此处应该打出,目前无值
		//oauth_token=c381a77ea3424c11902ac6721313c087&oauth_token_secret=4a7188d649c72b937d6049d1b57a329b&oauth_callback_confirmed=true
