...
Use \Gate\Package\User\UserConnect;
Use \Gate\Package\User\User;
Use \Gate\Package\Session\UserSession;
Use \Gate\Package\User\LogOn;
Use \Gate\Package\Medal\MedalLib;
Use \Gate\Package\User\Helper\RedisUserOauth;
...
class ConnectLib {
    ...
    protected $profileParams = array('user_id', 'nickname', 'email', 'ctime', 'password', 'active_code', 'cookie', 'is_actived', 'invite_code', 'last_logindate', 'status', 'realname', 'reg_from',  'level',  'login_times');
	protected $outSites = array('renren' => 1, 'weibo' => 3, 'qzone' => 4, 'baidu' => 5, 'taobao' => 6, 'wangyi' => 7, 'txweibo' => 8, 'douban' => 10, 'qplus' => 11);
	public function getCallback($connectType, $params){
		$callback = BASE_URL . 'connect/connect?connect='.$connectType;
		if(isset($params['is_login_bind'])){
			$callback .= '&is_login_bind=1';
		}
			//$callback .= '&is_login_bind=1';
    ...
    public function userLogin($userId, $type, $firstVisit, $params) {
		...
		$gKey = $params['request']->COOKIE['MEI_GLOBAL_KEY'];
		...
		if ($baseInfo['is_actived'] < 0) {
			$redirectUrl['destUrl'] = 'guang/home';		
			return $redirectUrl;
		}
		...
		$cpc_landing = $params['request']->COOKIE['MEI_BOB_LANDING'];
		$origion_refer = $params['request']->COOKIE['ORIGION_REFER'];
        $logHandle = new \Gate\Libs\Base\GateLog('weiboRedirect', 'normal');
        ...
        if (!empty($cpc_landing) && isset($origion_refer)) {
            ...
            setcookie('ORIGION_REFER', FALSE, time() - 3600, DEFAULT_COOKIE_PATH, DEFAULT_COOKIE_DOMAIN);
            return $redirectUrl;
        }
        ...
        if($firstVisit === FALSE) {
			...
			$meiliRefer = $params['request']->COOKIE['MEI_ORIGION'];
			$redirectUrl = array();
            if (isset($origion_refer) && $origion_refer != 'ihome') {
            ...
            elseif ($meiliRefer) {
				...
				//$log = new \Gate\Libs\Base\GateLog('meilirefer', 'normal');
				//$log->w_log(print_r($meiliRefer, true));
				$redirectUrl['new_comer'] = 0;
                setcookie('MEI_ORIGION', FALSE, time() - 3600, DEFAULT_COOKIE_PATH, DEFAULT_COOKIE_DOMAIN);
                return $redirectUrl;
            }
            else {
				//$logHandle = new \Gate\Libs\Base\GateLog('connectRedirect', 'normal');
				$redirectUrl['destUrl'] = 'ihome'; //$this->parseUrl($params['request']->refer);		
				$redirectUrl['new_comer'] = 0;
                return $redirectUrl;
            }
            return FALSE;
	...
	private function setGlobalKey($userId, $gKeyDB, $gKey, $seashell) {
        ...
        if (empty($gKeyDB)) {
            ...
            if (isset($gKey) && $gKey != 'Array') { 
                ...
                $globalKey = \Gate\Libs\Base\Utilities::getGlobalKey($seashell);
            }    
            $user->setGlobalKey($globalKey, $userId, $_SERVER['REQUEST_TIME']);
            setcookie('MEI_GLOBAL_KEY', $globalKey, $_SERVER['REQUEST_TIME'] + 3600 * 24 * 365, DEFAULT_COOKIE_PATH, DEFAULT_COOKIE_DOMAIN);
        }    
        elseif ($gKeyDB == 'Array') {
			$globalKey = \Gate\Libs\Base\Utilities::getGlobalKey($seashell);
            ...
            setcookie('MEI_GLOBAL_KEY', $gKeyDB, $_SERVER['REQUEST_TIME'] + 3600 * 24 * 365, DEFAULT_COOKIE_PATH, DEFAULT_COOKIE_DOMAIN);
        } 
	}
	...
	private function parseUrl($url) {
		...
		if (strpos($url, 'http://') !== FALSE) {        
            ...
            $log = new \Gate\Libs\Base\GateLog('parse_url', 'normal');
