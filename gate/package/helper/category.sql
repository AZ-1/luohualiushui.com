	...
	//----- add child
	*/
	DROP PROCEDURE  `hitao_beauty_add_category_child` ;
	delimiter $
	CREATE DEFINER=`hitao_beauty`@`localhost` PROCEDURE `hitao_beauty_add_category_child`(IN `parent_cate_id` INT(10) UNSIGNED, IN `new_cate_name` CHAR(32))
		NO SQL
		BEGIN 
			SELECT @lNum := left_num, @rNum := right_num, @next_level := level, @top_id:=top_id FROM beauty_category WHERE id = parent_cate_id;
			SELECT @defLeftNum :=left_num FROM beauty_category WHERE id=2;

			UPDATE beauty_category SET right_num=right_num+2 WHERE right_num > @rNum AND right_num < @defLeftNum;
			UPDATE beauty_category SET left_num=left_num+2 WHERE left_num > @rNum AND left_num < @defLeftNum;
	...
	/*----- del ---*/
	DROP PROCEDURE  `hitao_beauty_del_category` ;
	delimiter $
	CREATE DEFINER=`hitao_beauty`@`localhost` PROCEDURE `hitao_beauty_del_category`( IN `cataId` INT )
		BEGIN 
	...
	/*---- move child */
	DROP PROCEDURE  `hitao_beauty_move_category_child` ;
	delimiter $
	CREATE DEFINER=`hitao_beauty`@`localhost` PROCEDURE `hitao_beauty_move_category_child`(IN `cate_id` INT, IN `to_parent_id` INT)
		NO SQL
		BEGIN
