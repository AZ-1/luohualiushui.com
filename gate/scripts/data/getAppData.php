...
//					preg_match('/id=href="(.*)"/Uis' , $appurlhtml[1] , $appurl);
					...
					$pic_name = $pic_name['basename'];
						$db = new db();
						$sql = "select idx from apkgame";
						echo count($db ->select($sql));exit;
	...
	/*						$name = pathinfo($appurl);
							...
							$idx = $isHas[0]['idx'];
							$sql = "select idx,appname,version,appurl,appsize from apkgame where idx='$idx'";
							$old = $db -> select($sql);
							$update = "";
							$isUpdate = false;
							foreach($old as $o):
								$info = "(id = $idx) 更新".$o['appname']."\n";
								...
								if($o['appurl'] != $appurl){
									if($isUpdate)
										$update .= ", appurl = '$appurl'";
									else
										$update .= "appurl = '$appurl'";
									$info .= "下载链接:\n".$o['appurl'] ."=> $appurl\n";
									$isUpdate = true;
								}
								if($o['appsize'] != $appsize){
									if($isUpdate)
										$update .= ", appsize = '$appsize'";
									else
										$update .= "appsize = '$appsize'";
									$info .= "apk大小:\n".$o['appsize'] ."=> $appsize\n";
									$isUpdate = true;
							...
							if($isUpdate){
								...
								$sql = "update apkgame set appurl='$appurl' , updatetime='$updatetime',appsize='$appsize',version='$version' where idx='$idx'";
								$db -> execute($sql);
							}
						}
					}
