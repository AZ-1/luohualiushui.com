...
class MailSend extends \Gate\Libs\Scripts {
	...
	public function run() {
		...
		$time = time();
		$mailData = MailQueue::getInstance()->getData();
		...
		if (!empty($mailData)) {
			...
			foreach ($mailData as $key => $value) {
				...
				$this->start_time  = date('n月j日H:i ', $time);
				...
				foreach ($user_info as $u => $user) {
					$mailData = array(
						...
						'meeting_title'		=> '会议邀请：'.$this->meeting_topic,
					);
					$sendResult = $this->send($mailData);
					//var_dump($sendResult);
					if (isset($sendResult['success']) && !empty($sendResult['success'])) {
						MailQueue::getInstance()->updateData(array($queue_id));
					}else{
		...
		echo date('Ymd H:i:s'),':MailSend:Done',"\n";
		return true;
	}
	...
	private function send($mail = array()) {
		...
		curl_setopt($ch, CURLOPT_URL, 'http://mail01.xxx.com/officesendmail');
