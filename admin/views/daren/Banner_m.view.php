...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/daren/add_banner_m" target="navTab"><span>添加焦点图</span></a></li>
			<li></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="article_id" rel="<?php echo $v->id ?>">
				...
				<td>
					<a class="edit" href="/daren/edit_banner_m?id=<?php echo $v->id ?>" target="navTab"><span>修改</span></a> 
					|| <a class="delete" href="/daren/del_banner_m?id=<?php echo $v->id ?>" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a>
					<?php if($v->is_online){?>
						|| <a class="delete" href="/daren/Is_online_banner_m?id=<?php echo $v->id ?>&online=0" target="ajaxTodo" ><span style='color:red'>下线</span></a>
					<?php }else{?>
						|| <a class="delete" href="/daren/Is_online_banner_m?id=<?php echo $v->id ?>&online=1" target="ajaxTodo" ><span style='color:green'>上线</span></a> 
					<?php }?>
				</td>
				<td>
...
$('.topicButton').live('click',function(){
	...
	var sort	= input.val();
	var url = '/daren/edit_banner_sort_m';
	var data = {
		'id':id,
		'sort':sort
	}
