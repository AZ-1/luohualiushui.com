...
class Check_article extends \Gate\Libs\Controller {
	...
	public function run() {
		...
		$this->view->PageTotal = $this->PageTotal();	
	}
	...
	private function getArticleById(){
		return Article::getInstance()->getArticleById($this->id,"article_id,title,content",0);
	}
	private function upCheck(){
		return Article::getInstance()->upCheck($this->id,$this->quality);
	}
	private function upReason(){
		return Article::getInstance()->upReason($this->id,$this->no_pass_reason);
	}
